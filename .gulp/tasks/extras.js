...
gulp.task('convert:extras', () => {
	return gulp.src(`${paths.extras}/**/*.yml`).pipe(
		$.flatmap(stream => {
			return stream
				.pipe(
					$.plumber(error => {
						console.log(
							'[convert:extras]'.bold.magenta +
								' There was an issue converting color extras:\n'
									.bold.red +
								error.message +
								'To fix this error:\nAdd Sublime Text to the `PATH` and then install "PackageDev" via "Package Control".\nOpen Sublime Text before running the task.'
									.bold.blue
						);
						this.emit('end');
					})
				...
				.pipe(
					$.exec(
						'subl "<%= file.path %>" && subl --command "convert_file"'
					)
				)
				.pipe($.exec.reporter());
		})
...
gulp.task('build:extras', ['clean:extras'], cb => {
	runSequence('process:extras', 'convert:extras', error => {
		if (error) {
			console.log(
				'\n[build:extras]'.bold.magenta +
					' There was an issue building extras:\n'.bold.red +
					error.message
			);
		} else {
			console.log(

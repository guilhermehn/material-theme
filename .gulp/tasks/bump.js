...
gulp.task('bump-env-version', () => {
	return gulp
		.src(`${paths.utils}/info.py`)
		.pipe(
			$.if(Object.keys(argv).length === 4, $.bump({ key: '__version__' }))
		)
...
gulp.task('bump', cb => {
	runSequence('bump-pkg-version', 'bump-env-version', error => {
		if (error) {
			console.log(
				'\n[bump]'.bold.magenta +
					' There was an issue bumping version:\n'.bold.red +
					error.message
			);
		} else {
			console.log(
